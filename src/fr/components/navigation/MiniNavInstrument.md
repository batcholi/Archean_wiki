...
# Description
Le MiniNavInstrument est une version compacte du [NavInstrument](NavInstrument.md). Il offre les memes fonctionnalites de navigation dans un format plus petit, ideal pour les tableaux de bord, les petits vehicules ou lorsque l'espace est limite.

Contrairement au NavInstrument de taille standard, le MiniNavInstrument n'a pas de mode "OFF" - il passe par defaut en mode Flight lorsqu'aucun mode n'est specifie.

# Usage
Le MiniNavInstrument necessite une alimentation basse tension (200 W).
Il offre les memes trois modes de fonctionnement que le NavInstrument :
- **Flight** : Navigation dans l'environnement d'un corps celeste
- **Orbit** : Navigation en orbite
- **Locator** : Localiser un objet ou un corps celeste

Pour basculer entre les modes, envoyez une valeur de mode sur le canal 6 via le port de donnees.
...
> Consultez la documentation du [NavInstrument](NavInstrument.md) pour des informations detaillees sur chaque sortie.
