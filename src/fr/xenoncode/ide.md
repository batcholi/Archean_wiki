...
- **Éditeur de nœuds** : Un système de programmation alternatif basé sur des nœuds visuels pour créer des scripts de manière visuelle et intuitive.
...
Les programmes peuvent être écrits avec le système de nœuds ou en code pur, tandis que les fichiers normaux ne peuvent être écrits qu'en code pur.
...
## Basculer entre les modes (nœuds visuels / code)
Lorsque vous créez un programme, il commencera avec le mode nœuds visuels et quelques nœuds IO initiaux comme exemple que vous pouvez supprimer.
...
Pour le rendre modifiable, vous pouvez "Convert to Code", ce qui commentera simplement la première ligne #NODES qui définit les nœuds, et vous pourrez modifier le code.
Après avoir modifié en code, si vous voulez revenir aux nœuds, votre code sera écrasé par les nœuds que vous aviez avant de passer initialement au code.
...
Chaque fois que le joueur éditeur fait un changement, même non sauvegardé, les autres joueurs verront le changement se produire en temps réel, que ce soit en mode Code ou Nœuds.
Les joueurs non-éditeurs sont en lecture seule, ils ne peuvent pas modifier le code ou les nœuds, jusqu'à ce qu'ils cliquent sur Edit, auquel cas ils deviendront l'éditeur courant et l'éditeur précédent deviendra un lecteur en lecture seule.
...
- `CTRL+D` Sélectionner l'occurrence suivante dans le code, ou dupliquer le(s) nœud(s) sélectionné(s) en mode Nœuds
- `CTRL+C` Copier le code ou les nœuds sélectionnés
- `CTRL+V` Coller le code ou les nœuds
