...
# Description
MiniNavInstrument — компактная версия [NavInstrument](NavInstrument.md). Обеспечивает ту же навигационную функциональность в меньшем форм-факторе, идеально подходит для приборных панелей, малых транспортных средств или ограниченного пространства.

В отличие от полноразмерного NavInstrument, MiniNavInstrument не имеет режима "OFF" — по умолчанию он работает в режиме Flight, если режим не указан.

# Usage
MiniNavInstrument требует низковольтного питания (200 Вт).
Он предлагает те же три режима работы, что и NavInstrument:
- **Flight**: навигация в среде небесного тела
- **Orbit**: навигация на орбите
- **Locator**: определение местоположения объекта или небесного тела

Для переключения между режимами отправьте значение режима в канал 6 через порт данных.
...
> Подробную информацию о каждом выходном параметре см. в документации [NavInstrument](NavInstrument.md).
