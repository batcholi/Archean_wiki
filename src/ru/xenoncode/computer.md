...
Для включения переопределения экрана:
1. Откройте меню GetInfo компонента с помощью `V`
2. Установите флажок **Override Screen**
3. Обратитесь к экрану в вашем коде, используя `screen(ioIndex, channel)`, и рисуйте на нём как на любом другом экране

### IO
Computer может разрешать псевдонимы компонентов напрямую без необходимости Router. Просто используйте имя псевдонима в качестве первого параметра в функциях IO.
...
Позволяет создавать виртуальный экран с помощью XenonCode. Этот экран, определённый кодом, не является физическим и служит в основном для хранения визуальных данных. Затем вы можете копировать эти данные на физический экран с помощью функции `screen_copy()`, что быстрее, чем перерисовка.

Типичное использование: хранение спрайтов на виртуальном экране для копирования их всех сразу на физический экран. Например, создайте сложный фон и скопируйте его за один раз вместо перерисовки каждый раз.

Избегайте рисования на виртуальном экране в цикле, так как это будет столь же медленно, как рисование непосредственно на физическом экране. Хотя это может быть полезно в некоторых случаях, мы рекомендуем использовать его статическим образом.
