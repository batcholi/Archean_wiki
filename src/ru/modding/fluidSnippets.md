...
Вот несколько примеров скриптов XenonCode, демонстрирующих работу с жидкостями при моддинге с помощью XenonCode.

В следующих примерах мы используем макрос Slider для управления компонентами. Его можно оставить как есть в пользовательском интерфейсе или заменить другим источником ввода (например, компонент Throttle, кнопка-переключатель и т.д.).
```xc
#INFO slider "slider" 0 -1 1 "throttle"
```

Компонент, использованный для создания этих примеров, имеет входной порт жидкости с именем "fluid-in" и выходной порт жидкости с именем "fluid-out".
Эти примеры предоставлены для справки и могут быть адаптированы под ваши нужды.
