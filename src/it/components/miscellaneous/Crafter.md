...
# Description
Il Crafter e' un componente che permette la fabbricazione rapida di oggetti.

# Usage
Il Crafter richiede alimentazione ad alta tensione e consuma 500 watt in standby e 10 kW quando operativo.

Puo' essere controllato manualmente tramite il suo touchscreen integrato e/o tramite la sua porta dati per un controllo piu' avanzato.
...
> #### Informazioni:
>- Quando il `progress` nei dati di uscita e' `-1`, significa che la ricetta non puo' essere eseguita a causa di risorse mancanti o la fabbricazione richiesta non esiste nel gioco.
>- Il Crafter mostra un avviso **"Not enough power"** se la fabbricazione fallisce a causa di potenza insufficiente.
>- Anche se il Crafter ha solo due porte di fluido, puo' utilizzare tutti i fluidi necessari per le ricette utilizzando le [Fluid Junction](../fluids/FluidJunction.md), che possono essere collegate a tutte le sorgenti di fluido necessarie. Il Crafter le utilizzera' semplicemente quando necessario.
>- `Fluid Levels` e' un oggetto key-value contenente valori tra 0 e 1 per `h2`, `o2` e `h2o`
>
> #### Suggerimenti:
> - E' possibile utilizzare semplicemente un [Toggle Button](../controllers/ToggleButton.md) per avviare la fabbricazione continua finche' il pulsante e' attivo e ci sono risorse sufficienti nell'inventario collegato.
>

# Go further with the crafter:
## Simple Auto-Crafting Setup

Il Crafter supporta nativamente un comportamento di auto-fabbricazione quando configurato correttamente.

Per abilitarlo, basta:
- Collegare le porte di ingresso e uscita oggetti del Crafter allo stesso contenitore.
- Collegare la porta dati dello stesso contenitore direttamente alla porta dati del Crafter (canale 0).

Questa configurazione permette al Crafter di gestire automaticamente ricette complesse con sotto-fabbricazioni prelevando gli ingredienti direttamente dal contenitore.

Finche' il contenitore contiene tutte le materie prime necessarie, il Crafter:
- Fabbrichera' automaticamente i sotto-componenti mancanti.
- Poi assemblera' il prodotto finale.

Questo e' il modo piu' semplice per ottenere un sistema di auto-fabbricazione senza scrivere codice personalizzato.

> - In questa configurazione, poiche' la porta dati e' gia' collegata al contenitore, non sara' possibile utilizzare un [Toggle Button](../controllers/ToggleButton.md) per abilitare la fabbricazione continua. Invece, bisognera' utilizzare lo schermo integrato per avviare le fabbricazioni, una alla volta.
> - Se si desidera implementare una vera fabbricazione continua in questa configurazione, sara' necessario un approccio piu' avanzato utilizzando XenonCode per controllare il Crafter.

## Continuous Auto-Crafting from a Computer

E' possibile controllare la fabbricazione automatica continua interamente da un Computer utilizzando il **canale 2**:

- **Channel 0**: Impostare a `1` per abilitare la fabbricazione continua
- **Channel 1**: Impostare il nome della ricetta da fabbricare
- **Channel 2**: Collegare l'uscita dati del Container per abilitare l'auto-fabbricazione con sotto-fabbricazioni ricorsive

Questo consente il controllo programmatico completo del Crafter beneficiando comunque della logica di fabbricazione ricorsiva automatica.
