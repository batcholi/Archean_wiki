...
Un Computer può ispezionare qualsiasi componente che è in grado di indirizzare, compresi quelli dietro a dei router.
...
L'oggetto chiave-valore restituito contiene:

| Membro | Descrizione |
|---|---|
| `.id` | identificatore univoco del componente, stabile per tutto il tempo in cui esiste |
| `.alias` | il suo alias, o un testo vuoto se non ne ha |
| `.port` | la porta IO a cui è collegato, oppure `-1` se si trova dietro a un router |
| `.type` | il suo tipo di componente |
| `.temp` | la sua temperatura attuale, in kelvin |
| `.damage` | il suo livello di danno, da 0 a 100 |
| `.broken` | `1` se è completamente danneggiato e non funziona più, altrimenti `0` |
| `.x` `.y` `.z` | la sua posizione in metri, relativa al build su cui è montato |
| `.size_x` `.size_y` `.size_z` | la sua dimensione in metri |
| `.mass` | la sua massa in chilogrammi |

### Screen Override
Un Computer puo' sovrascrivere lo schermo di qualsiasi componente connesso ad esso. Questo permette di sostituire il display predefinito di un componente con grafiche personalizzate renderizzate dal tuo programma XenonCode.

Per abilitare la sovrascrittura dello schermo:
1. Apri il menu GetInfo del componente con `V`
2. Seleziona l'opzione **Override Screen**
3. Fai riferimento allo schermo nel tuo codice usando `screen(ioIndex, channel)` e disegna su di esso come su qualsiasi altro schermo

### IO
I Computer possono risolvere gli alias dei componenti direttamente senza bisogno di un Router. Usa semplicemente il nome dell'alias come primo parametro nelle funzioni IO.
...
Permette la creazione di uno schermo virtuale con XenonCode. Questo schermo, definito tramite codice, non e' fisico e serve principalmente a memorizzare dati visivi. Puoi poi copiare questi dati su uno schermo fisico usando la funzione `screen_copy()`, che e' piu' veloce del ridisegno.

Uso tipico: memorizzare sprite in uno schermo virtuale per copiarli tutti in una volta su uno schermo fisico. Per esempio, creare uno sfondo complesso e copiarlo in un colpo solo invece di ridisegnarlo ogni volta.

Evita di disegnare sullo schermo virtuale in un ciclo, poiche' sarebbe lento quanto disegnare direttamente su uno schermo fisico. Anche se questo puo' essere utile in alcuni casi d'uso, raccomandiamo di usarlo in modo statico.
