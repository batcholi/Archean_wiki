...
## Introduction
Il sistema HUD (Heads-Up Display) permette ai giocatori di creare le proprie interfacce grafiche per visualizzare informazioni sullo schermo come testi, pulsanti, disegni... usando XenonCode.

Gli HUD sono interamente gestiti dal client, il che significa che ogni giocatore ha i propri HUD e gli altri giocatori non possono vederli. Tuttavia, possono comunicare con [Beacon](../components/navigation/Beacon.md) o altri giocatori attraverso un sistema integrato che permette di inviare e ricevere dati su frequenze.

E' importante notare che poiche' gli HUD sono completamente lato client, saranno disponibili su tutti i server/mondi a cui ti connetti. Non puoi impostarli per un server/mondo specifico se non manualmente come indicato nella sezione seguente.

> Informazioni aggiuntive:
> - Quando si usa un pulsante o qualsiasi altra interazione su un HUD, il `tasto destro` e' preferibile per evitare di catturare la visuale del mouse.

## Creating a HUD
Poiche' gli HUD sono una funzionalita' completamente lato client, non hanno oggetti direttamente associati nel gioco. Per creare un HUD, devi andare nel menu delle impostazioni di gioco `F1` e nella scheda `HUD`.

![HUD Menu](hud-img/hudMenu.png)

Da questo menu, puoi creare quanti HUD desideri e attivarli/disattivarli a tuo piacimento usando la casella di controllo. E' importante sapere che ogni HUD e' un'istanza unica e non comunica nativamente con altri HUD, sebbene la comunicazione sia possibile attraverso una funzionalita' che verra' spiegata piu' avanti in questa pagina.

Una volta creato il tuo HUD, puoi aprire il suo IDE per programmare le tue funzionalita'.

## Creating your first graphical interface
Gli HUD usano pannelli per visualizzare contenuti sullo schermo. Un pannello puo' contenere elementi grafici come testi, pulsanti, disegni...

Puoi creare quanti pannelli desideri e posizionarli sullo schermo come preferisci.
Per creare un pannello, la sintassi e' simile a quella che potresti trovare sullo schermo di una [dashboard](dashboard.md).
    
```xc
var $panel = panel(center, sizeX, sizeY)
```

Esempio di un HUD che visualizza "Hello World" al centro dello schermo in un riquadro grigio di 100x50 pixel
...
```
> E' importante ricordare che l'UI Scaling configurato nelle impostazioni di gioco influenza i valori restituiti da queste funzioni.
...
```
Nota: Il modo di disegnare sul pannello e' simile allo [schermo dashboard](../xenoncode/dashboard.md#screen-rendering-functions)
...
# Shared Values
Shared Values e' una funzionalita' che permette di recuperare e impostare informazioni nel client del giocatore.

Una lista di shared values e' nativamente disponibile per permetterti di recuperare informazioni sull'ambiente del giocatore.
...
E' possibile creare i propri shared values per trasmettere/ricevere, ad esempio, informazioni tra HUD.
```xc
...
Queste funzioni sono disponibili negli HUD e funzionano allo stesso modo che sui computer.
Fai riferimento alla documentazione per il [virtualscreen](../xenoncode/computer.md#virtual-screen-function) e lo [screen_copy](../xenoncode/computer.md#screen-rendering-functions-draw-on-a-virtual-screen).

# Examples
### HUD che visualizza la velocita' del giocatore
```xc
...
### HUD che punta un beacon e visualizza la distanza
```xc
