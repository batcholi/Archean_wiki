# Dashboard
## Introduction
Quando si usa il Dashboard Tool, vari elementi come schermi e label possono essere utilizzati per visualizzare informazioni, proprio come lo schermo di un [computer AIO](../components/computers/Computer.md). Tuttavia, come schermo esterno, il suo funzionamento puo' differire leggermente in certe situazioni.

## First Use of a Dashboard Screen/Label
### Communication with Dashboard Elements
Le dashboard, a differenza della maggior parte dei componenti disponibili nel gioco, usano una singola porta dati per accedere a tutti gli elementi disponibili sulla dashboard. Ogni elemento ha un canale di comunicazione unico e puo' essere usato per inviare e ricevere dati. Puoi recuperare il canale premendo il tasto `V` mentre punti l'elemento della dashboard con cui vuoi interagire.

*Nota: I canali sono assegnati in modo incrementale, il che significa che il primo elemento aggiunto a una dashboard avra' il canale 0, il secondo avra' il canale 1, e cosi' via. Se rimuovi un elemento, il canale verra' riassegnato al prossimo elemento aggiunto.*
### Creating and Using a Dashboard Screen/Label
Quando si usa lo schermo integrato di un [computer](../components/computers/Computer.md), puoi semplicemente eseguire azioni su di esso, come colorare lo schermo di rosso o visualizzare il testo "Hello World".
```xc
init
    blank(red)
    write(0, 0, white, "Hello World")
```
Nel contesto di una dashboard, devi prima definire lo schermo in una variabile per poter eseguire azioni su di esso.
```xc
...
Supponendo di aver assegnato la variabile `$screen` al nostro schermo dashboard, ecco la lista delle funzioni che puoi usare per interagire con esso.
