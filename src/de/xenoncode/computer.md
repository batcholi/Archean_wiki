...
## Spezifische Funktionen für Computer
...
Ein Computer kann jede Komponente inspizieren, die er adressieren kann, einschließlich derer hinter Routern.
...
Das zurückgegebene Key-Value-Objekt enthält:

| Member | Beschreibung |
|---|---|
| `.id` | eindeutige id der Komponente, stabil solange sie existiert |
| `.alias` | ihr Alias, oder ein leerer Text, wenn sie keinen hat |
| `.port` | der IO-Port, an dem sie angeschlossen ist, oder `-1`, wenn sie hinter einem Router liegt |
| `.type` | ihr Komponententyp |
| `.temp` | ihre aktuelle Temperatur, in Kelvin |
| `.damage` | ihr Schadensgrad, von 0 bis 100 |
| `.broken` | `1`, wenn sie vollständig beschädigt ist und nicht mehr funktioniert, sonst `0` |
| `.x` `.y` `.z` | ihre Position in Metern, relativ zum Build, auf dem sie montiert ist |
| `.size_x` `.size_y` `.size_z` | ihre Größe in Metern |
| `.mass` | ihre Masse in Kilogramm |

### Screen Override
Ein Computer kann den Bildschirm jeder angeschlossenen Komponente übernehmen. Dies ermöglicht es dir, die Standardanzeige einer Komponente durch benutzerdefinierte Grafiken zu ersetzen, die von deinem XenonCode-Programm gerendert werden.

Um Screen Override zu aktivieren:
1. Öffne das GetInfo-Menü der Komponente mit `V`
2. Aktiviere die Option **Override Screen**
3. Referenziere den Bildschirm in deinem Code mit `screen(ioIndex, channel)` und zeichne darauf wie auf jeden anderen Bildschirm

### IO
Computer können Komponenten-Aliase direkt auflösen, ohne einen Router zu benötigen. Verwende einfach den Aliasnamen als ersten Parameter in IO-Funktionen.
...
### Bildschirm-Renderfunktionen (auf einem virtuellen Bildschirm zeichnen)
```xc
...
### Virtuelle Bildschirm-Funktion
Ermöglicht die Erstellung eines virtuellen Bildschirms mit XenonCode. Dieser durch Code definierte Bildschirm ist nicht physisch und dient hauptsächlich zur Speicherung visueller Daten. Du kannst diese Daten dann mit der `screen_copy()`-Funktion auf einen physischen Bildschirm kopieren, was schneller ist als neu zu zeichnen.

Typische Verwendung: Sprites in einem virtuellen Bildschirm speichern, um sie alle auf einmal auf einen physischen Bildschirm zu kopieren. Zum Beispiel einen komplexen Hintergrund erstellen und ihn in einem Rutsch kopieren, anstatt ihn jedes Mal neu zu zeichnen.

Vermeide das Zeichnen auf dem virtuellen Bildschirm in einer Schleife, da dies genauso langsam wäre wie das direkte Zeichnen auf einem physischen Bildschirm. Auch wenn dies in einigen Anwendungsfällen nützlich sein kann, empfehlen wir die statische Verwendung.
